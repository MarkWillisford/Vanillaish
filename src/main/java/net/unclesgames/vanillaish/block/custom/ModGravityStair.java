...
public class ModGravityStair extends FallingBlock implements Waterloggable {
  ...
  protected static final VoxelShape[] TOP_SHAPES = ModGravityStair.composeShapes(TOP_SHAPE, BOTTOM_NORTH_WEST_CORNER_SHAPE, BOTTOM_NORTH_EAST_CORNER_SHAPE, BOTTOM_SOUTH_WEST_CORNER_SHAPE, BOTTOM_SOUTH_EAST_CORNER_SHAPE);
  protected static final VoxelShape[] BOTTOM_SHAPES = ModGravityStair.composeShapes(BOTTOM_SHAPE, TOP_NORTH_WEST_CORNER_SHAPE, TOP_NORTH_EAST_CORNER_SHAPE, TOP_SOUTH_WEST_CORNER_SHAPE, TOP_SOUTH_EAST_CORNER_SHAPE);
  private static final int[] SHAPE_INDICES = new int[]{12, 5, 3, 10, 14, 13, 7, 11, 13, 7, 11, 14, 8, 4, 1, 2, 4, 1, 2, 8};
  private final Block baseBlock;
  private final BlockState baseBlockState;

  public ModGravityStair(BlockState baseBlockState, AbstractBlock.Settings settings) {
    super(settings);
    this.setDefaultState((BlockState)((BlockState) ((BlockState)((BlockState)this.stateManager.getDefaultState()).with(FACING, Direction.NORTH)).with(HALF, BlockHalf.BOTTOM).with(SHAPE, StairShape.STRAIGHT)).with(WATERLOGGED, false));
  ...
  private static VoxelShape[] composeShapes(VoxelShape base, VoxelShape northWest, VoxelShape northEast, VoxelShape southWest, VoxelShape southEast) {
    return (VoxelShape[]) IntStream.range(0, 16).mapToObj(i -> ModGravityStair.composeShape(i, base, northWest, northEast, southWest, southEast)).toArray(VoxelShape[]::new);
  }
...
//    this.baseBlock.scheduledTick(state, world, pos, random);
  ...
  public BlockState getPlacementState(ItemPlacementContext ctx) {
    ...
    return (BlockState)blockState.with(SHAPE, ModGravityStair.getStairShape(blockState, ctx.getWorld(), blockPos));
  }
  ...
  public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
    ...
    if (direction.getAxis().isHorizontal()) {
      return (BlockState)state.with(SHAPE, ModGravityStair.getStairShape(state, world, pos));
    }
    world.createAndScheduleBlockTick(pos, this, this.getFallDelay());
  ...
  private static StairShape getStairShape(BlockState state, BlockView world, BlockPos pos) {
    ...
    if (StairsBlock.isStairs(blockState) && state.get(HALF) == blockState.get(HALF) && (direction2 = blockState.get(FACING)).getAxis() != state.get(FACING).getAxis() && ModGravityStair.isDifferentOrientation(state, world, pos, direction2.getOpposite())) {
    ...
    if (StairsBlock.isStairs(blockState2) && state.get(HALF) == blockState2.get(HALF) && (direction3 = blockState2.get(FACING)).getAxis() != state.get(FACING).getAxis() && ModGravityStair.isDifferentOrientation(state, world, pos, direction3)) {
